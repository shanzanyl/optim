...
const StatusBadge = ({ status }: { status: string | null }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-black uppercase border ${cfg[s] || cfg.Warning}`}>
      <span className={`w-2 h-2 rounded-full animate-pulse ${dot[s] || dot.Warning}`} />
      {s}
    </span>
  );
};
    ...
    <div className="p-6 sm:p-8 bg-[#1e2f50] rounded-2xl border border-[#3b4f6e]">
        ...
        <div className={`bg-[#1e2f50] p-6 sm:p-8 rounded-2xl border transition-all ${imageStatus === 'success' ? 'border-emerald-500/50 bg-emerald-500/5' :
          ...
          <div className="flex bg-[#0f1a2e] p-1 rounded-xl mb-6 border border-[#3b4f6e]">
            <button
              ...
              className={`flex-1 py-2.5 text-base font-semibold rounded-lg transition-all ${activeInputMethod === 'ocr'
            ...
            <button
              ...
              className={`flex-1 py-2.5 text-base font-semibold rounded-lg transition-all ${activeInputMethod === 'manual'
                ? 'bg-blue-600 text-white shadow-lg'
                : 'text-slate-400 hover:text-white'
                }`}
            >
          ...
          {activeInputMethod === 'ocr' ? (
            <>
              ...
              {!isOcrParsed ? (
                <label
                  ...
                  className={`w-full px-6 py-2.5 rounded-xl cursor-pointer flex items-center justify-center transition-all font-semibold text-white text-base ${imageStatus === 'uploading' ? 'bg-slate-600 cursor-wait' : 'bg-blue-600 hover:bg-blue-500 shadow-lg shadow-blue-600/20'}`}
                >
                ...
                <button
                  ...
                  className="w-full px-6 py-2.5 rounded-xl bg-slate-600 hover:bg-slate-500 transition-all font-semibold text-white text-base"
                >
                  Upload Ulang
                </button>
              )}
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleClassify(); }} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-bold text-white tracking-widest mb-1.5 block">Prx (dBm)</label>
                  <input type="number" step="0.01" value={manualForm.prx} onChange={e => setManualForm({ ...manualForm, prx: e.target.value })} placeholder="0.0" className="w-full px-3 py-2 bg-[#0f1a2e] border border-[#3b4f6e] rounded-lg text-white text-sm focus:ring-2 focus:ring-blue-500/50 outline-none placeholder:text-slate-600 font-mono" />
                </div>
                <div>
                  <label className="text-sm font-bold text-white tracking-widest mb-1.5 block">Avg-Total (dB/km)</label>
                  <input type="number" step="0.001" value={manualForm.avg_total} onChange={e => setManualForm({ ...manualForm, avg_total: e.target.value })} placeholder="0.0" className="w-full px-3 py-2 bg-[#0f1a2e] border border-[#3b4f6e] rounded-lg text-white text-sm focus:ring-2 focus:ring-blue-500/50 outline-none placeholder:text-slate-100 font-mono" />
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-[#3b4f6e]/30">
                    {[1, 2, 3, 4].map(km => (
                      <tr key={km} className="hover:bg-[#0f1a2e]/20">
                        ...
                        <td className="p-2">
                          <input type="number" step="0.00001" value={manualForm[`distance_${km}` as keyof typeof manualForm]} onChange={e => setManualForm({ ...manualForm, [`distance_${km}`]: e.target.value })} placeholder="0.0" className="w-full px-2 py-1.5 bg-[#0f1a2e]/50 border border-[#3b4f6e] rounded text-white text-sm font-mono outline-none" />
                        </td>
                        <td className="p-2">
                          <input type="number" step="0.001" disabled={km === 4} value={km === 4 ? '' : manualForm[`loss_${km}` as keyof typeof manualForm]} onChange={e => setManualForm({ ...manualForm, [`loss_${km}`]: e.target.value })} placeholder={km === 4 ? '—' : '0.0'} className="w-full px-2 py-1.5 bg-[#0f1a2e]/50 border border-[#3b4f6e] rounded text-white text-sm font-mono outline-none disabled:opacity-45" />
                        </td>
                        <td className="p-2">
                          <input type="number" step="0.001" value={manualForm[`total_l_${km}` as keyof typeof manualForm]} onChange={e => setManualForm({ ...manualForm, [`total_l_${km}`]: e.target.value })} placeholder="0.0" className="w-full px-2 py-1.5 bg-[#0f1a2e]/50 border border-[#3b4f6e] rounded text-white text-sm font-mono outline-none" />
                        </td>
                        <td className="p-2">
                          <input type="number" step="0.001" value={manualForm[`avg_l_${km}` as keyof typeof manualForm]} onChange={e => setManualForm({ ...manualForm, [`avg_l_${km}`]: e.target.value })} placeholder="0.0" className="w-full px-2 py-1.5 bg-[#0f1a2e]/50 border border-[#3b4f6e] rounded text-white text-sm font-mono outline-none" />
                        </td>
                        <td className="p-2">
                          <input type="number" step="0.01" value={manualForm[`return_${km}` as keyof typeof manualForm]} onChange={e => setManualForm({ ...manualForm, [`return_${km}`]: e.target.value })} placeholder="0.0" className="w-full px-2 py-1.5 bg-[#0f1a2e]/50 border border-[#3b4f6e] rounded text-white text-sm font-mono outline-none" />
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <button
                ...
                className={`w-full px-6 py-2.5 rounded-xl flex items-center justify-center transition-all font-semibold text-white text-base mt-4 gap-2 ${imageStatus === 'uploading' ? 'bg-slate-600 cursor-wait' : isOcrParsed ? 'bg-emerald-600 hover:bg-emerald-500 shadow-lg shadow-emerald-600/20' : 'bg-blue-600 hover:bg-blue-500 shadow-lg shadow-blue-600/20'}`}
              >
          ...
          {isOcrParsed && ocrParseResult && !lastResult && (
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">DETECTED VALUES</p>
              ...
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs text-white ml-1">(input manual)</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                {[1, 2, 3, 4].map((i) => (
                  <div key={`loss-${i}`} className="bg-[#0f1a2e]/50 rounded-lg p-2 flex justify-between items-center hover:bg-[#0f1a2e] transition-colors">
                    <span className="text-white font-medium">Loss km {i} <span className="text-white text-sm">(dB)</span></span>
                    <div className="flex items-center gap-1.5">
                      <input
                        ...
                        className={`w-16 px-2 py-0.5 bg-[#0f1a2e] border border-[#3b4f6e] rounded text-white text-right font-mono text-sm outline-none focus:ring-1 focus:ring-blue-500/50 ${i === 4 ? 'opacity-45 cursor-not-allowed' : ''}`}
                      />
                    </div>
                  </div>
                ))}
              ...
              <div className="grid grid-cols-2 gap-2 text-sm mt-1">
                {[1, 2, 3, 4].map((i) => (
                  <div key={`total-${i}`} className="bg-[#0f1a2e]/50 rounded-lg p-2 flex justify-between items-center hover:bg-[#0f1a2e] transition-colors">
                    <span className="text-white font-medium">Total-L km {i} <span className="text-white text-sm">(dB)</span></span>
                    <div className="flex items-center gap-1.5">
                      <input
                        type="text"
                        inputMode="decimal"
                        ...
                        className="w-16 px-2 py-0.5 bg-[#0f1a2e] border border-[#3b4f6e] rounded text-white text-right font-mono text-sm outline-none focus:ring-1 focus:ring-blue-500/50"
                      />
                    </div>
                  </div>
                ))}
              ...
              <div className="grid grid-cols-2 gap-2 text-sm mt-1">
                {[1, 2, 3, 4].map((i) => (
                  <div key={`avg-${i}`} className="bg-[#0f1a2e]/50 rounded-lg p-2 flex justify-between items-center hover:bg-[#0f1a2e] transition-colors">
                    <span className="text-white font-medium">Avg-L km {i} <span className="text-white text-sm">(dB/km)</span></span>
                    <div className="flex items-center gap-1.5">
                      <input
                        type="text"
                        inputMode="decimal"
              ...
              <div className="bg-[#0f1a2e]/70 rounded-lg p-2 flex justify-between items-center text-sm border border-[#3b4f6e]/50">
                <span className="text-white font-bold">Avg-Total<span className="text-white text-sm ml-1">(dB/km)</span></span>
                <div className="flex items-center gap-1.5">
                  <input
                    type="text"
                    inputMode="decimal"
              ...
              <div className="grid grid-cols-2 gap-2 text-sm mt-1">
                {[1, 2, 3, 4].map((i) => (
                  <div key={`ret-${i}`} className="bg-[#0f1a2e]/50 rounded-lg p-2 flex justify-between items-center hover:bg-[#0f1a2e] transition-colors">
                    <span className="text-white font-medium">Return Km {i} <span className="text-white text-sm">(dB)</span></span>
                    <div className="flex items-center gap-1.5">
                      <input
                        type="text"
                        inputMode="decimal"
              ...
              <button
                ...
                disabled={imageStatus === 'uploading'}
                className="w-full mt-4 px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 transition-all font-semibold text-white text-base flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/20 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {imageStatus === 'uploading' ? (
                  <>
                    <RefreshCw size={16} className="animate-spin" />
          ...
          {lastResult && imageStatus === 'success' && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <p className="text-sm font-bold text-white uppercase tracking-widest">Detected Values</p>
                ...
                {lastResult.extracted.total_ls && (
                  <div className="grid grid-cols-2 gap-2.5 text-sm mt-2">
              ...
              <button onClick={() => setShowRawOcr(!showRawOcr)} className="text-xs text-slate-500 hover:text-slate-400 underline transition">
                {showRawOcr ? '▲ Hide' : '▼ Show'} raw OCR text
              </button>
              {showRawOcr && (
                <pre className="text-xs bg-[#0f1a2e] border border-[#3b4f6e] rounded-lg p-3 text-slate-400 overflow-x-auto max-h-32 whitespace-pre-wrap">
                  {lastResult.raw_text}
                </pre>
              )}
            </div>
      ...
      <div className="bg-[#1e2f50] border border-[#3b4f6e] rounded-[2.5rem] shadow-2xl overflow-hidden">
        <div className="p-5 border-b border-[#3b4f6e] flex justify-between items-center">
          <h2 className="text-base font-bold text-white uppercase tracking-widest">History of Measurements</h2>
          <div className="flex items-center gap-3">
            <span className="text-sm text-slate-400 bg-[#0f1a2e] px-3 py-1.5 rounded-full border border-[#3b4f6e]">
        ...
        <div className="overflow-x-auto max-h-[500px] overflow-y-auto">
          <table className="w-full text-left">
            <thead className="sticky top-0 bg-[#1e2f50] z-10">
              <tr className="bg-[#1e2f50] text-white text-sm font-black tracking-wider border-b border-[#3b4f6e]">
                <th className="px-6 py-4 text-center bg-[#1e2f50]">TIME</th>
                <th className="px-6 py-4 text-center bg-[#1e2f50]">LOSS Km 1-4 (dB)</th>
                <th className="px-6 py-4 text-center bg-[#1e2f50]">TOTAL-L (dB)</th>
                <th className="px-6 py-4 text-center bg-[#1e2f50]">RETURN Km 1-4 (dB)</th>
                <th className="px-6 py-4 text-center bg-[#1e2f50]">Prx (dBm)</th>
                <th className="px-6 py-4 bg-[#1e2f50]">CLASSIFICATION</th>
                <th className="px-6 py-4 text-center bg-[#1e2f50]">STATUS</th>
              </tr>
            </thead>
                ...
                displayedHistory.map((row, idx) => {
                  ...
                  return (
                    <tr key={row.id || idx} className="hover:bg-[#2a3d60]/20 transition-colors">
                      <td className="px-6 py-4 text-center text-white text-sm font-mono">{recordTime}</td>
                      <td className="px-6 py-4 text-center text-white text-sm font-mono">
                        {formatLossValue(row.loss_1)} | {formatLossValue(row.loss_2)} |{' '}
                        {formatLossValue(row.loss_3)} | {formatLossValue(row.loss_4)}
                      </td>
                      <td className="px-6 py-4 text-center text-emerald-400 font-bold text-sm font-mono">
                        {totalLDisplay}
                      </td>
                      <td className="px-6 py-4 text-center text-white text-sm font-mono">
                      ...
                      <td className="px-6 py-4 text-center text-blue-400 font-bold text-sm font-mono">
                        {row.prx != null ? `${row.prx.toFixed(2)} dBm` : '—'}
                      </td>
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1.5 rounded-full text-sm font-bold border ${row.klasifikasi === 'Normal' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' :
