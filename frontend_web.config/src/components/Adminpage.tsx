...
const Adminpage: React.FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const pendingRes = await fetch('http://localhost:8000/api/admin/users', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const pendingData = await pendingRes.json();
      setPendingUsers(pendingData.users || []);
      
      const allRes = await fetch('http://localhost:8000/api/admin/users/all', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleMakeAdmin = async (userId: number, email: string) => {
    if (confirm(`Jadikan ${email} sebagai admin?`)) {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://localhost:8000/api/admin/make-admin/${userId}`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (response.ok) {
